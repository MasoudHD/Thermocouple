...
#include "stdio.h"
     ...
     * @param voltage           The TC voltage (must be multiply by 1000)
     * @param cjc               Temperature of the Cold Junction (in °C and in the range of -20~70°C and must be be multiply by 1000)
     * @param tu                The desired  unit of the TC temperature (selectable in TempUnit enum)
     * @return int              Return the TC temperature according to the desired unit (multiplied by 1000)
     */
    int getTcTemp(TcType type, long int voltage, int cjc,TempUnit tu)
    {
        ...
        switch(type)
        {
            case TC_TYPE_K:
            {
                    ...
                    tc = TC_TB_K_TYPE;
                    len = TC_TB_K_TYPE_LEN;
                    minVoltage = TC_TB_K_MIN;
                    maxVoltage = TC_TB_K_MAX;
                #endif
                break;
            }
            default:
        ...
        long int cjcMinIndex = cjc/1000 - minVoltage;
        long int cjcMaxIndex = cjcMinIndex+1;
        long int diffVolt = tc[cjcMaxIndex] - tc[cjcMinIndex];
        long int diffIndex = (cjcMaxIndex+minVoltage)*1000-cjc;
        int vCJC = tc[cjcMaxIndex] - diffIndex*diffVolt/1000;
        int compensate = voltage+vCJC;
        for(int i=0; i<len; i++)
        {
            if(compensate <= tc[i])
            {
                return  ((compensate-tc[i-1])*1000/(tc[i]- tc[i-1]))+((i-1+TC_TB_K_MIN)*1000);
            }
        }
        return 0xFFFFFF;
    }
    ...
    /**
     * @brief                   Calculate the voltage of the ADC (multiplied by 1000)
     * 
     * @param hTcAdc            ADC struct of the TC 
     */
    void readTcVoltage(TcAdcHandler* hTcAdc)
    {
        hTcAdc->Voltage = (int)(((hTcAdc->AdcValue*hTcAdc->Vref)/hTcAdc->Resolution)-hTcAdc->Offset)*1000/hTcAdc->Gain;
    }
